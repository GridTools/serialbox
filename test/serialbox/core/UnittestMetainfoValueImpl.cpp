//===-- serialbox/core/UnittestMetainfoValueImpl.cpp ------------------------------------*- C++ -*-===//
//
//                                    S E R I A L B O X
//
...
//
/// \file
/// This file implements the unittests of the MetainfoValueImpl.
///
//===------------------------------------------------------------------------------------------===//
...
namespace {
template <class T>
class MetainfoValueImplTest : public testing::Test {};
...
TYPED_TEST_CASE(MetainfoValueImplTest, TestTypes);

TYPED_TEST(MetainfoValueImplTest, Constrcution) {
  // Default construct
  MetainfoValueImpl m;
  ...
  MetainfoValueImpl value1(TypeParam(pair.first));
  MetainfoValueImpl value2(TypeParam(pair.second));
  ...
  MetainfoValueImpl value1_lref(v1_lref);
...
TYPED_TEST(MetainfoValueImplTest, Comparison) {
  auto pair = getValuePair<TypeParam>();
  MetainfoValueImpl value1(TypeParam(pair.first));
  MetainfoValueImpl value2(TypeParam(pair.second));
  ...
  if(std::is_same<TypeParam, std::string>::value) {
    MetainfoValueImpl bool_value(bool(true));
    EXPECT_FALSE(value1 == bool_value);
  } else {
    MetainfoValueImpl string_value(std::string("str"));
    EXPECT_FALSE(value1 == string_value);
  }
}

TYPED_TEST(MetainfoValueImplTest, toString) {
  auto pair = getValuePair<TypeParam>();
  MetainfoValueImpl value1(TypeParam(pair.first));
  MetainfoValueImpl value2(TypeParam(pair.second));
