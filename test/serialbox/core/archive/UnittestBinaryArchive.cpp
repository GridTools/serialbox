...
TYPED_TEST(BinaryArchiveReadWriteTest, WriteAndRead) {
  ...
  {
    BinaryArchive archiveWrite(OpenModeKind::Write, this->directory->path().string(), "field");
    EXPECT_FALSE(SB_FILESYSTEM::exists(this->directory->path() / ("field_u.dat")));
    EXPECT_FALSE(SB_FILESYSTEM::exists(this->directory->path() / ("field_v.dat")));
    EXPECT_FALSE(SB_FILESYSTEM::exists(this->directory->path() / ("field_storage_2d.dat")));
    EXPECT_FALSE(SB_FILESYSTEM::exists(this->directory->path() / ("field_storage_7d.dat")));
  }
}
