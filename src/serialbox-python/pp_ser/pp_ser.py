    ...
    if os.path.isdir(src):
        ...
        filtered_list.extend(
            [
                os.path.join(src, f)
                for f in files
                if os.path.isfile(os.path.join(src, f)) and file_filter(f)
            ]
        )
        dirs = [f for f in files if os.path.isdir(os.path.join(src, f))]
        for d in dirs:
            build_tree(
                os.path.join(src, d), os.path.join(dest, d), filtered_list, file_filter
            )
    else:
...
class PpSer:
    def __init__(
        ...
        self.module = (
            module  # name of Fortran module which contains serialization methods
        )
        ...
        self.sp_as_var = (
            sp_as_var  # Syntax for savepoints using variable instead of string
        )
        ...
        self.modes = {"write": 0, "read": 1, "read-perturb": 2, "CPU": 0, "GPU": 1}
    ...
    def __ser_arg_parse(self, args):
        ...
        for arg in args[1:]:
            ...
            if if_encountered:
                if if_statement:
                    self.__exit_error(
                        directive=args[0], msg="IF statement must be last argument"
                    )
                if_statement = arg
            else:
                val = arg.split("=")
    ...
    def __ser_register(self, args):
        ...
        if len(dirs) < 2:
            self.__exit_error(
                directive=args[0], msg="Must specify a name, a type and the field sizes"
            )
        ...
        if dirs[1] not in datatypes:
            self.__exit_error(
                directive=args[0],
                msg='Data type "{0}" not understood. Valid types are: {1}'.format(
                    dirs[1], ", ".join('"' + d + '"' for d in datatypes.keys())
                ),
            )
    ...
    def __ser_zero(self, args):
        (dirs, keys, values, if_statement) = self.__ser_arg_parse(args)
        if len(keys) > 0:
            self.__exit_error(directive=args[0], msg="Must specify a list of fields")
        ...
        for k, v in zip(keys, values):
            ...
            if not any(ext in v for ext in self.__computed_fields_sign):
                l += (
                    ...
                    + "    "
                    + "call "
                    + self.methods["dataread"]
                    + "(ppser_serializer_ref, ppser_savepoint, '"
                    + k
                    + "', "
                    + v
                    + ")\n"
    ...
    def __re_module(self):
        r = re.compile(
            "^ *(?P<statement>module|program) +(?P<identifier>[a-z][a-z0-9_]*)",
            re.IGNORECASE,
        )
        ...
        if m:
            if m.group(1):
                ...
                if args[0].upper() in self.language["init"]:
                    ...
                    self.__exit_error(
                        directive=args[0], msg="Unknown directive encountered"
                    )
        return m
    ...
    def __re_endmodule(self):
        ...
        if m:
            ...
            if m.group(2) and self.__module.lower() != m.group(2).lower():
                self.__exit_error(
                    msg='Was expecting "end ' + m.group(1) + " " + self.__module + '"'
                )
            self.__module = ""
    ...
    def __check_intent_in(self, line):
        ...
        var_with_dim = [
            x.strip().replace(" ", "") for x in re.split(r",(?![^(]*\))", lhs)
        ]
        ...
        self.intentin_removed.extend(
            [x for x in fields_in_this_line if x not in self.intentin_removed]
        )
                ...
                while nextline:
                    ...
                    if nextline.find("&") != -1:
                        ...
                        nextline = linecache.getline(
                            os.path.join(self.infile), lookahead_index
                        )
                    else:
                        nextline = None
    ...
    def lexer(self, final=False):
        ...
        if final:
            ...
            if self.__ser:
                self.__exit_error(
                    msg="Unterminated #ifdef " + self.ifdef + " encountered"
                )
            if self.__module:
...
def parse_args():
    ...
    parser.add_option(
        "-i",
        "--ignore-identical",
        help="Ignore files which are not modified by pre-processor",
        default=False,
        action="store_true",
        dest="ignore_identical",
    )
    parser.add_option(
        "-d",
        "--output-dir",
        help="The target directory for writing pre-processed files",
        default="",
        type=str,
        dest="output_dir",
    )
    parser.add_option(
        "-o",
        "--output",
        help="Output file name to preprocess single file",
        default="",
        type=str,
        dest="output_file",
    )
    parser.add_option(
        "-r",
        "--recursive",
        help="Recursively process target directory and mirror tree",
        default=False,
        action="store_true",
        dest="recursive",
    )
    ...
    parser.add_option(
        "-p",
        "--no-prefix",
        help="Don't generate preprocessing macro definition for ACC_PREFIX",
        default=True,
        action="store_false",
        dest="acc_prefix",
    )
    parser.add_option(
        "-a",
        "--acc-if",
        help="Add IF clause to OpenACC update statement",
        default="",
        type=str,
        dest="acc_if",
    )
    parser.add_option(
        "-m",
        "--module",
        help="Extra MODULE to be add to the use statement",
        default="",
        type=str,
        dest="modules",
    )
    parser.add_option(
        "-s",
        "--sp-as-var",
        help="Savepoint specified as variable instead of string",
        default=False,
        action="store_true",
        dest="sp_as_var",
    )
...
if __name__ == "__main__":
    ...
    for infile in args:
        if options.output_dir:
            if options.recursive:
                outfile = os.path.join(
                    options.output_dir,
                    os.path.sep.join(
                        [p for p in os.path.dirname(infile).rsplit(os.path.sep) if p][
                            1:
                        ]
                    ),
        ...
        if os.path.exists(outfile) and os.path.getctime(outfile) > os.path.getctime(
            infile
        ):
            ...
            ser = PpSer(
                ...
                real="wp",
                outfile=outfile,
                identical=(not options.ignore_identical),
                verbose=options.verbose,
                acc_prefix=options.acc_prefix,
                acc_if=options.acc_if,
                modules=options.modules,
                sp_as_var=options.sp_as_var,
            )
            ser.preprocess()
