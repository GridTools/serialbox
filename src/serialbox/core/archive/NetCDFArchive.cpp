...
FieldID NetCDFArchive::write(const StorageView& storageView, const std::string& field,
  ...
  SB_FILESYSTEM::path filename = directory_ / (prefix_ + "_" + field + ".nc");
...
void NetCDFArchive::read(StorageView& storageView, const FieldID& fieldID,
  ...
  SB_FILESYSTEM::path filename = directory_ / (prefix_ + "_" + fieldID.name + ".nc");
...
void NetCDFArchive::readFromFile(std::string filename, StorageView& storageView,
  ...
  if(!SB_FILESYSTEM::exists(filename))
...
void NetCDFArchive::clear() {
  ...
  for(SB_FILESYSTEM::directory_iterator it(directory_); it != end; ++it) {
    if(SB_FILESYSTEM::is_regular_file(it->path()) &&
       ...
       SB_FILESYSTEM::path(it->path()).extension() == ".nc") {
