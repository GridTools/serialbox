//===-- serialbox/core/frontend/gridtools/MetainfoMapImpl.h -------------------------*- C++ -*-===//
//
//                                    S E R I A L B O X
//
...
public:
  ...
  /// \brief Type of an entry of the MetainfoMapImpl (`std::pair<std::string, meta_info_value>`)
  ...
  meta_info_map() : map_impl_(std::make_shared<MetainfoMapImpl>()) {}
  ...
  meta_info_map(std::initializer_list<value_type> list)
      : map_impl_(std::make_shared<MetainfoMapImpl>(list)) {}
  ...
  const std::shared_ptr<MetainfoMapImpl>& impl() const { return map_impl_; }

private:
  std::shared_ptr<MetainfoMapImpl> map_impl_;
};
